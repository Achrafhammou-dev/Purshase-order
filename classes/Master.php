...
if (!empty($po_no)) {
    ...
    $year = date("Y");
    $po_no = "";
    $last_po_query = $this->conn->query("SELECT `po_no` FROM `po_list` WHERE `po_no` LIKE '{$year}-%' ORDER BY `po_no` DESC LIMIT 1");
    
    if ($last_po_query->num_rows > 0) {
        $last_po = $last_po_query->fetch_assoc()['po_no'];
        // Extract the sequence number and increment it
        $last_sequence = (int)substr($last_po, strpos($last_po, '-') + 1);
        $new_sequence = $last_sequence + 1;
    } else {
        // Reset sequence to 1 for the new year
        $new_sequence = 1;
    }
    
    // Format the new PO number
    $po_no = sprintf("%s-%04d", $year, $new_sequence);
}
